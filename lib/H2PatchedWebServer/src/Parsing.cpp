...
static size_t readRawBodyChunk(WiFiClient& client, uint8_t* buffer,
  ...
  while (received < requested) {
    ...
    if (available > 0) {
      ...
      const int count = client.read(buffer + received, take);
      if (count > 0) {
...
bool WebServer::_parseRequest(WiFiClient& client) {
  ...
  if (method == HTTP_POST || method == HTTP_PUT || method == HTTP_PATCH || method == HTTP_DELETE){
    ...
    if (!isForm && _currentHandler && _currentHandler->canRaw(_currentUri)){
      log_v("Parse raw");
      // WiFiClient::setTimeout() uses seconds in Arduino-ESP32 2.0.17. The
      // WebServer client object can otherwise retain the 5 s response timeout
      // from a previous request, exactly colliding with H2 Gauge's loop TWDT.
      ...
      while (!rawAborted &&
        ...
        // Content-Length. On H2 Gauge loopTask this collided with the 5 s TWDT
        // before RAW_WRITE/RAW_END could run. Never read past the declared body.
